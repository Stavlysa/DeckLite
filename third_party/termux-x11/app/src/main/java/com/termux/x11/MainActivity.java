...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (SDK_INT >= VERSION_CODES.R) {
            // ACTION_DOWN-only unbuffering above does not cover hover or captured
            // mouse events. Request these classes on both possible event targets.
            // Keep gamepad/joystick dispatch unchanged; older Android keeps the
            // existing per-touch-stream fallback.
            int pointerSources = InputDevice.SOURCE_CLASS_POINTER |
                    InputDevice.SOURCE_CLASS_POSITION | InputDevice.SOURCE_CLASS_TRACKBALL;
            lorieParent.requestUnbufferedDispatch(pointerSources);
            lorieView.requestUnbufferedDispatch(pointerSources);
        }
