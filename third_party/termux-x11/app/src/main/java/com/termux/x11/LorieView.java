...
public class LorieView extends SurfaceView implements InputStub {
    ...
    // A transient, private, same-UID bridge from the container's X11 watcher.
    // It changes presentation only: neither the X screen nor the Wine game is
    // resized. The existing GPU draw and its input matrix use the same viewport.
    private final Rect deckliteSource = new Rect();
    private final ColorDrawable[] deckliteBars = {
            new ColorDrawable(Color.BLACK), new ColorDrawable(Color.BLACK),
            new ColorDrawable(Color.BLACK), new ColorDrawable(Color.BLACK)};
    private volatile boolean decklitePolling;
    private HandlerThread deckliteWorker;
    private Handler deckliteIo;
    private android.view.View deckliteOverlayHost;
    private final Runnable decklitePoll = new Runnable() {
        @Override public void run() {
            if (!decklitePolling) return;
            Rect next = new Rect();
            File capability = new File(getContext().getFilesDir(), "tmp/decklite-viewport.support");
            try {
                if (!capability.isFile()) {
                    try (FileOutputStream stream = new FileOutputStream(capability)) { stream.write('1'); }
                }
                capability.setLastModified(System.currentTimeMillis());
            } catch (Exception ignored) { /* Missing bridge means safe fallback. */ }
            File state = new File(getContext().getFilesDir(), "tmp/decklite-viewport.state");
            long age = System.currentTimeMillis() - state.lastModified();
            if (age >= -1000 && age < 2500 && state.length() > 0 && state.length() < 192) {
                try (FileInputStream stream = new FileInputStream(state)) {
                    byte[] bytes = new byte[192];
                    int size = stream.read(bytes);
                    String[] fields = new String(bytes, 0, size, UTF_8).trim().split("\\s+");
                    if (fields.length == 7 && fields[0].equals("1")) {
                        int rw = Integer.parseInt(fields[1]), rh = Integer.parseInt(fields[2]);
                        int x = Integer.parseInt(fields[3]), y = Integer.parseInt(fields[4]);
                        int w = Integer.parseInt(fields[5]), h = Integer.parseInt(fields[6]);
                        if (rw == p.x && rh == p.y && rw <= 32767 && rh <= 32767 &&
                                x >= 0 && y >= 0 && w >= 100 && h >= 100 &&
                                w <= rw && h <= rh && x <= rw-w && y <= rh-h)
                            next.set(x, y, x+w, y+h);
                    }
                } catch (Exception ignored) { /* Partial/stale data restores normal view. */ }
            }
            post(() -> {
                if (!decklitePolling) return;
                if (next.right > p.x || next.bottom > p.y) next.setEmpty();
                if (!deckliteSource.equals(next)) {
                    deckliteSource.set(next);
                    rendererZoom = 100;
                    setRendererZoom(100);
                    updateViewport();
                    Log.i("DeckLiteViewport", "source=" + next + " root=" + p.x + "x" + p.y);
                }
            });
            if (decklitePolling && deckliteIo != null) deckliteIo.postDelayed(this, 500);
        }
    };

    @Override protected void onAttachedToWindow() {
        super.onAttachedToWindow();
        deckliteOverlayHost = (android.view.View) getParent();
        for (ColorDrawable bar : deckliteBars) deckliteOverlayHost.getOverlay().add(bar);
        decklitePolling = true;
        deckliteWorker = new HandlerThread("DeckLiteViewportIO");
        deckliteWorker.start();
        deckliteIo = new Handler(deckliteWorker.getLooper());
        deckliteIo.post(decklitePoll);
    }

    @Override protected void onDetachedFromWindow() {
        decklitePolling = false;
        if (deckliteIo != null) deckliteIo.removeCallbacksAndMessages(null);
        if (deckliteWorker != null) deckliteWorker.quitSafely();
        deckliteIo = null;
        deckliteWorker = null;
        deckliteSource.setEmpty();
        if (deckliteOverlayHost != null)
            for (ColorDrawable bar : deckliteBars) deckliteOverlayHost.getOverlay().remove(bar);
        deckliteOverlayHost = null;
    ...
    private void updateViewport() {
        ...
        for (ColorDrawable bar : deckliteBars) bar.setBounds(0, 0, 0, 0);
        if (!deckliteSource.isEmpty() && deckliteSource.right <= p.x && deckliteSource.bottom <= p.y) {
            float scale = Math.min((float) availableW / deckliteSource.width(),
                                  (float) availableH / deckliteSource.height());
            // Avoid absurd/corrupt geometry overflowing common GLES viewport
            // limits. Keep the normal safe desktop if the fit is not usable.
            if (p.x * scale > 16384 || p.y * scale > 16384) {
                deckliteSource.setEmpty();
                updateViewport();
                return;
            }
            int gameW = Math.round(deckliteSource.width() * scale);
            int gameH = Math.round(deckliteSource.height() * scale);
            int gameX = availableLeft + (availableW - gameW) / 2;
            int gameY = availableTop + (availableH - gameH) / 2;
            int vx = gameX - Math.round(deckliteSource.left * scale);
            int vy = gameY - Math.round(deckliteSource.top * scale);
            viewport.set(vx, vy, vx + Math.round(p.x * scale), vy + Math.round(p.y * scale));
            deckliteBars[0].setBounds(0, 0, gameX, surfaceH);
            deckliteBars[1].setBounds(gameX + gameW, 0, surfaceW, surfaceH);
            deckliteBars[2].setBounds(gameX, 0, gameX + gameW, gameY);
            deckliteBars[3].setBounds(gameX, gameY + gameH, gameX + gameW, surfaceH);
            for (ColorDrawable bar : deckliteBars) {
                Rect bounds = new Rect(bar.getBounds());
                bounds.offset(getLeft(), getTop());
                bar.setBounds(bounds);
            }
        }
        if (deckliteOverlayHost != null) deckliteOverlayHost.invalidate();
    ...
    public InputConnection onCreateInputConnection(EditorInfo outAttrs) {
        ...
        outAttrs.imeOptions = EditorInfo.IME_FLAG_NO_FULLSCREEN | EditorInfo.IME_ACTION_SEND;
        return mConnection;
    }
